...
/**
 * The YCSB binding for <a href="https://github.com/Legend147/HashKV">HashKV</a>.
 */
public class HashKVClient extends DB {

  private static HashKV db = null;

  private static synchronized void getDBInstance() {
    if (db == null) {
      try {
        db = new HashKV("hashkv_database");
      } catch (Exception e) {
  ...
  public Status scan(String table, String startkey, int recordcount,
    ...
    try {
      Hashkv.KVPair queryResult =  db.scan(startkey.getBytes(), recordcount);

      for (int i=0;i<queryResult.keys.length;i++) {
        String key = queryResult.keys[i];
        if (fields == null || fields.contains(key)) {
          HashMap<String, String> value;
          value = (HashMap<String, String>) bytesToMap(queryResult.values[i]);
